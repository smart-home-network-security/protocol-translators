...
from test.utils import dicts_equal
...
def test_parse_forward_aliases() -> None:
    ...
    expected = {
        ...
        "nfq": [
            {
                "template": '( compare_ip((ip_addr_t) {{.version = 6, .value.ipv6 = dst_addr}}, interactions_data[{}].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "{}"), (ip_addr_t) {{.version = 6, .value.ipv6 = dst_addr}}) )',
...
def test_parse_backward_aliases() -> None:
    ...
    expected = {
        ...
        "nfq": [
            {
                "template": '( compare_ip((ip_addr_t) {{.version = 6, .value.ipv6 = src_addr}}, interactions_data[{}].cached_ip) || dns_entry_contains(dns_map_get(dns_map, "{}"), (ip_addr_t) {{.version = 6, .value.ipv6 = src_addr}}) )',
                "match": "www.example.com"
            }
        ]
    }
